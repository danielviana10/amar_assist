    ...
    <v-card>
      ...
      <v-card-text>
        <v-form ref="formRef" @submit.prevent="saveProductChanges">
          <v-container>
            <v-text-field v-model="editingProduct.title" label="Título*" :rules="titleRules" required counter
              maxlength="40" variant="outlined" class="mb-2" />

            <v-textarea v-model="editingProduct.description" label="Descrição*" rows="3" :rules="descriptionRules"
              variant="outlined" required counter maxlength="200" class="mb-2"
              @input="editingProduct.description = sanitizeDescription(editingProduct.description)" />
            ...
            <v-row class="mt-1">
              <v-col cols="6">
                <v-text-field v-model="editingProduct.price" label="Preço*" type="number" prefix="R$" min="0" step="10"
                  :rules="priceRules(editingProduct.cost)" required class="mb-2" variant="outlined" />
              </v-col>
              <v-col cols="6">
                <v-text-field v-model="editingProduct.cost" label="Custo*" type="number" prefix="R$" min="0" step="10"
                  :rules="costRules(editingProduct.price)" required class="mb-2" variant="outlined" />
              </v-col>
            </v-row>

            <v-file-input v-model="newImages" multiple accept="image/jpeg,image/png,image/jpg"
              label="Adicionar imagens (apenas JPG/PNG/JPEG)" prepend-icon="" prepend-inner-icon="mdi-camera" show-size
            ...
            <v-row class="mt-2">
              <v-col cols="12">
                ...
                <template v-else>
                  <v-row v-if="productImages.length > 0" class="d-flex flex-wrap ga-4 justify-center">
                    <v-col v-for="(image, index) in productImages" :key="image.id" cols="auto"
                      class="d-flex justify-center" style="width: 120px; max-width: 120px;">
                      <v-card width="100" class="position-relative">
                        <v-overlay :model-value="image.isDeleting" contained class="align-center justify-center"
                          scrim="black" persistent>
                          <v-progress-circular indeterminate color="white" size="30"></v-progress-circular>
                        </v-overlay>

                        <v-img :src="getFullImageUrl(image.path)" height="100" cover
                          :style="{ opacity: image.isDeleting ? 0.5 : 1 }" />

                        <v-btn icon size="x-small" color="error" class="position-absolute" style="top: 4px; right: 4px"
                          @click="deleteImage(image.id)" :disabled="image.isDeleting">
      ...
      <v-card-actions>
        ...
        <v-btn color="primary" variant="text" @click="saveProductChanges" :loading="loadingSave"
